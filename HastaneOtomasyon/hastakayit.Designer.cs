namespace HastaneOtomasyon
{
    partial class hastakayit
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            label1 = new Label();
            label2 = new Label();
            label3 = new Label();
            label4 = new Label();
            label5 = new Label();
            textBoxHastaTc = new TextBox();
            textBoxHastaAdSoyad = new TextBox();
            comboBoxBolum = new ComboBox();
            comboBoxDoktor = new ComboBox();
            dateTimePickerRandevuTarih = new DateTimePicker();
            buttonRandevuOlustur = new Button();
            SuspendLayout();
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Location = new Point(56, 30);
            label1.Name = "label1";
            label1.Size = new Size(74, 15);
            label1.TabIndex = 0;
            label1.Text = "TC Numarası";
            // 
            // label2
            // 
            label2.AutoSize = true;
            label2.Location = new Point(56, 75);
            label2.Name = "label2";
            label2.Size = new Size(57, 15);
            label2.TabIndex = 1;
            label2.Text = "Ad Soyad";
            // 
            // label3
            // 
            label3.AutoSize = true;
            label3.Location = new Point(56, 120);
            label3.Name = "label3";
            label3.Size = new Size(42, 15);
            label3.TabIndex = 2;
            label3.Text = "Bölüm";
            // 
            // label4
            // 
            label4.AutoSize = true;
            label4.Location = new Point(56, 165);
            label4.Name = "label4";
            label4.Size = new Size(43, 15);
            label4.TabIndex = 3;
            label4.Text = "Doktor";
            // 
            // label5
            // 
            label5.AutoSize = true;
            label5.Location = new Point(56, 210);
            label5.Name = "label5";
            label5.Size = new Size(80, 15);
            label5.TabIndex = 4;
            label5.Text = "Rabdevu saati";
            // 
            // textBoxHastaTc
            // 
            textBoxHastaTc.Location = new Point(169, 22);
            textBoxHastaTc.Name = "textBoxHastaTc";
            textBoxHastaTc.Size = new Size(200, 23);
            textBoxHastaTc.TabIndex = 5;
            // 
            // textBoxHastaAdSoyad
            // 
            textBoxHastaAdSoyad.Location = new Point(169, 67);
            textBoxHastaAdSoyad.Name = "textBoxHastaAdSoyad";
            textBoxHastaAdSoyad.Size = new Size(200, 23);
            textBoxHastaAdSoyad.TabIndex = 6;
            // 
            // comboBoxBolum
            // 
            comboBoxBolum.FormattingEnabled = true;
            comboBoxBolum.Location = new Point(169, 117);
            comboBoxBolum.Name = "comboBoxBolum";
            comboBoxBolum.Size = new Size(200, 23);
            comboBoxBolum.TabIndex = 7;
            comboBoxBolum.SelectedIndexChanged += comboBoxBolum_SelectedIndexChanged;
            // 
            // comboBoxDoktor
            // 
            comboBoxDoktor.FormattingEnabled = true;
            comboBoxDoktor.Location = new Point(169, 162);
            comboBoxDoktor.Name = "comboBoxDoktor";
            comboBoxDoktor.Size = new Size(200, 23);
            comboBoxDoktor.TabIndex = 8;
            // 
            // dateTimePickerRandevuTarih
            // 
            dateTimePickerRandevuTarih.CustomFormat = "";
            dateTimePickerRandevuTarih.Location = new Point(169, 204);
            dateTimePickerRandevuTarih.Name = "dateTimePickerRandevuTarih";
            dateTimePickerRandevuTarih.Size = new Size(200, 23);
            dateTimePickerRandevuTarih.TabIndex = 9;
            // 
            // buttonRandevuOlustur
            // 
            buttonRandevuOlustur.Location = new Point(240, 249);
            buttonRandevuOlustur.Name = "buttonRandevuOlustur";
            buttonRandevuOlustur.Size = new Size(129, 45);
            buttonRandevuOlustur.TabIndex = 10;
            buttonRandevuOlustur.Text = "Randevu Oluştur";
            buttonRandevuOlustur.UseVisualStyleBackColor = true;
            buttonRandevuOlustur.Click += buttonRandevuOlustur_Click;
            // 
            // hastakayit
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            BackColor = SystemColors.ActiveCaption;
            ClientSize = new Size(402, 322);
            Controls.Add(buttonRandevuOlustur);
            Controls.Add(dateTimePickerRandevuTarih);
            Controls.Add(comboBoxDoktor);
            Controls.Add(comboBoxBolum);
            Controls.Add(textBoxHastaAdSoyad);
            Controls.Add(textBoxHastaTc);
            Controls.Add(label5);
            Controls.Add(label4);
            Controls.Add(label3);
            Controls.Add(label2);
            Controls.Add(label1);
            Name = "hastakayit";
            Text = "hastakayit";
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private Label label1;
        private Label label2;
        private Label label3;
        private Label label4;
        private Label label5;
        private TextBox textBoxHastaTc;
        private TextBox textBoxHastaAdSoyad;
        private ComboBox comboBoxBolum;
        private ComboBox comboBoxDoktor;
        private DateTimePicker dateTimePickerRandevuTarih;
        private Button buttonRandevuOlustur;
    }
}